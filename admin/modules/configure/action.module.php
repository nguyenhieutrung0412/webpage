...
if($request['module'] == 'content'){
	...
	foreach($cat_fields as  $rs){
		if($rs['Comment'] != 'system'){
			...
			$rs['number'] = $data['cat_fields'][$rs['code']]['number']?$data['cat_fields'][$rs['code']]['number']:0;
			$rs['type'] = $oClass->input_type('cat_fields['.$rs['code'].'][type]', $data['cat_fields'][$rs['code']]['type'],true,' onchange="fields_status(this);"');
			$rs['status_default'] = $oClass->input_status_default('cat_fields['.$rs['code'].'][status_default]', $data['cat_fields'][$rs['code']]['status_default']);
