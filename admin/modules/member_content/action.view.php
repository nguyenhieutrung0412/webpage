...
defined('_ROOT') or die(__FILE__);
	$cond =1;
	$_SESSION['matinhtrang'] = $_REQUEST['locdonhang'];
	if($_REQUEST['locdonhang'])
	{
		
		$ma = $_SESSION['matinhtrang'];
		$cond .= " AND hd.matinhtrang=".$ma;
	}
if($_POST){
	...
	$tpl->setfile(array(
		'body'=>$cfg_type['tpl_view']?$cfg_type['tpl_view']:'member_content.tpl',
	));
	$fields = $oMaster->fields('hoadon');
	$status_fields = '';
	...
	while($rs = $ps->fetch()){
		$rs = $hook->format($rs);
		if($_REQUEST['locdonhang']==$rs['matinhtrang'])
	...
	while($rs = $cat->fetch()){
		...
		$delcat = 1;
		if($cfg_type['nodelcat_ids'] && in_array($rs['madonhang'],explode(',',$cfg_type['nodelcat_ids']))) $delcat = 0;
	...
	$sort_default = $cfg_type['sort_default']?$cfg_type['sort_default']:'mahoadon';
	...
	if($request['by'] && in_array($request['by'],$fields)) $sort_order = 'hd.'.$request['by'].' '.$request['order'];
